...
class DatasetManager:
    ...
    def update_dataset(self, dataset, custom=False):
    ...
    def load_datasets(self):
        self.train_dataset = self.dataset_class(self.params, "train", self.params["train"]["name"], self.get_paths_and_sets("train", self.params["train"]["datasets"]),custom=False)
        ...
        for custom_name in self.params["valid"].keys():
            self.valid_datasets[custom_name] = self.dataset_class(self.params, "valid", custom_name, self.get_paths_and_sets("valid", self.params["valid"][custom_name]),custom=False)
    ...
    def generate_test_loader(self, custom_name, sets_list, custom=False):
        if custom_name in self.test_loaders.keys():
            return

        if custom:
            paths_and_sets = sets_list[0][1]
        else:
            paths_and_sets = list()
        ...
        self.test_datasets[custom_name] = self.dataset_class(self.params, "test", custom_name, paths_and_sets, custom=custom)
        if self.dataset_class is OCRDataset:
            self.update_dataset(self.test_datasets[custom_name],custom=custom)
...
class GenericDataset(Dataset):
    ...
    def __init__(self, params, set_name, custom_name, paths_and_sets, from_segmentation=False, custom=False):
        ...
        if from_segmentation:
            ...
            if custom:
                self.samples = self.load_custom_test_samples(paths_and_sets)
            else:
    ...
    @staticmethod
    def load_custom_test_samples(path):

        samples = list()
        filenames = os.listdir(path)
        for file in filenames:
            with Image.open(os.path.join(path, file)) as pil_img:
...
class OCRDataset(GenericDataset):
    ...
    def __init__(self, params, set_name, custom_name, paths_and_sets,custom=False):
        super(OCRDataset, self).__init__(params, set_name, custom_name, paths_and_sets,custom=custom)
        self.charset = None
        self.tokens = None
...
class OCRCustomCollateFunction:
    """
    Merge samples data to mini-batch data for OCR task
    """
