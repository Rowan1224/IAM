...
from seq_modeling.helpers import bert_tokenize
from seq_modeling.models import SubwordBert
...
def model_inference_custom(model, data, topk, device, batch_size=16, vocab_=None):
    """
    model: an instance of SubwordBert
    data: list of tuples, with each tuple consisting of correct and incorrect 
            sentence string (would be split at whitespaces)
    ...
    data_iter = batch_iter_custom(data, batch_size=VALID_batch_size, shuffle=False)
    model.eval()
    model.to(device)
    predictions = {}
    for batch_id, (batch_names, batch_sentences) in tqdm(enumerate(data_iter)):
        torch.cuda.empty_cache()
        st_time = time.time()
        # set batch data for bert
        # print(batch_sentences)
        batch_sentences_, batch_bert_inp, batch_bert_splits = bert_tokenize(batch_sentences)
        if len(batch_sentences_) == 0:
            for name, sen in zip(batch_names, batch_sentences):
                predictions[name] = sen
            continue
        else:
            batch_sentences = batch_sentences_
        batch_bert_inp = {k: v.to(device) for k, v in batch_bert_inp.items()}

        _, batch_lengths = labelize(batch_sentences, vocab)
        try:
            with torch.no_grad():
                """
                NEW: batch_predictions can now be of shape (batch_size,batch_max_seq_len,topk) if topk>1, else (batch_size,batch_max_seq_len)
                """
                batch_predictions = model(batch_bert_inp, batch_bert_splits, targets=None, topk=topk)
        except RuntimeError:
