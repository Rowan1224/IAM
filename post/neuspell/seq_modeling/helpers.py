...
def batch_iter_custom(data, batch_size, shuffle):
    """
    each data item is a tuple of lables and text
    """
    n_batches = int(np.ceil(len(data) / batch_size))
    ...
    for i in range(n_batches):
        batch_indices = indices[i * batch_size: (i + 1) * batch_size]
        batch_sentences = [data[idx][1] for idx in batch_indices]
        batch_names = [data[idx][0] for idx in batch_indices]

        yield (batch_names,batch_sentences)
...
def bert_tokenize(batch_sentences,bert_pretrained_name_or_path=None):
    """
    inputs:
