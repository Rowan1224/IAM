...
from seq_modeling.helpers import batch_iter, labelize, progressBar, batch_accuracy_func
...
class BertChecker(Corrector):
    ...
    def evaluate(self, clean_file, corrupt_file, data_dir=""):
        ...
        # batch_size = 1 if self.device == "cpu" else 1
        for x, y, z in zip([data_dir], [clean_file], [corrupt_file]):
            print(x, y, z)
            test_data = load_data(x, y, z)
            pred = model_inference(self.model,
                                test_data,
                                topk=1,
                                device=self.device,
                                batch_size=1,
    ...
    def finetune(self,
        ...
        TRAIN_BATCH_SIZE, VALID_BATCH_SIZE = 16, 4
        GRADIENT_ACC = 4
        DEVICE = self.device
        START_EPOCH, N_EPOCHS = 0, n_epochs
        CHECKPOINT_PATH = os.path.join(self.ckpt_path if self.ckpt_path else data_dir, "models",
